...
class Edictum:
    ...
    def from_yaml(cls, *paths, **kwargs):
        ...
        return _from_yaml(cls, *paths, **kwargs)

    @classmethod
    def from_yaml_string(cls, content, **kwargs):
        """Create an Edictum instance from a YAML string or bytes."""
        from edictum._factory import _from_yaml_string

        return _from_yaml_string(cls, content, **kwargs)
    ...
    def from_template(cls, name, **kwargs):
        ...
        return _from_template(cls, name, **kwargs)

    @classmethod
    def list_templates(cls, template_dirs=None):
    ...
    @classmethod
    async def from_server(cls, url, api_key, agent_id, **kwargs):
        """Create an Edictum instance wired to a remote edictum-server."""
        from edictum._server_factory import _from_server

        return await _from_server(cls, url, api_key, agent_id, **kwargs)

    async def run(self, tool_name, args, tool_callable, *, session_id=None, **envelope_kwargs):
        """Framework-agnostic entrypoint."""
        from edictum._runner import _run

        return await _run(self, tool_name, args, tool_callable, session_id=session_id, **envelope_kwargs)

    def evaluate(self, tool_name, args, **kwargs):
        """Dry-run evaluation of a tool call against all matching contracts."""
        from edictum._dry_run import _evaluate

        return _evaluate(self, tool_name, args, **kwargs)
    ...
    async def _start_sse_watcher(self):
    ...
    async def _stop_sse_watcher(self):
